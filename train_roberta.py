...
CONFIG = {
    ...
    "batch_size": 8,
    "learning_rate": 2e-5,
    "epochs": 5,           # 3-5 as requested, setting 5 with early stopping
...
def load_and_split_data(file_path: str) -> DatasetDict:
    ...
    # Git LFS Pointer check (If file is suspiciously small, < 10KB)
...
def generate_markdown_report(metrics: dict, config: dict, output_path: str):
    ...
    report = f"""# RoBERTa Fine-Tuning Report for AI-Generated Text Detection
...
def main():
    ...
    # Use parallel processes to speed up mapping
    ...
    # Use adamw_torch_fused if CUDA is available for speedups
    fused_optim = "adamw_torch_fused" if device.type == "cuda" else "adamw_torch"
    training_args = TrainingArguments(
        ...
        gradient_checkpointing=True,         # VRAM optimization for RTX 3050 (4GB)
        optim=fused_optim,                   # Fused kernel optimization
        label_smoothing_factor=0.1,          # Calibration improvement
    ...
    report_path = f"{CONFIG['reports_dir']}/roberta_report.md"
