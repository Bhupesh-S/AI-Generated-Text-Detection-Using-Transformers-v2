...
dataset.py
==========
Dataset loading, splitting, and tokenization for the RoBERTa pipeline.
...
def tokenize_datasets(
    ...
    Padding strategy rationale (profiling result, n=2000):
      - Mean token length: 426  |  Median: 407
      - 79% of texts EXCEED max_length=256 → truncated to exactly 256
      - Only 7% fit within 128 tokens
      Conclusion: dynamic padding yields <1% benefit here because 79% of
      batches are already at max density. Static padding at 256 is optimal.
      The DataCollatorWithPadding (pad_to_multiple_of=8) in trainer.py still
      applies tensor-core alignment for the 21% of shorter sequences.
    ...
    tokenized = dataset_dict.map(
        _tokenize,
        batched=True,
        batch_size=1000,                # Process 1000 samples per map batch (cache efficiency)
