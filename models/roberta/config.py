...
config.py
=========
Central configuration for the RoBERTa Fine-Tuning Pipeline.
...
Optimized Settings for RTX 3050 Laptop (4 GB VRAM):
  - batch_size set to 8 with gradient_accumulation_steps set to 2
    (effective batch size = 16, identical optimization trajectory and accuracy).
...
# ---------------------------------------------------------------------------
# Base project root (two levels up from models/roberta/)
# ---------------------------------------------------------------------------
...
@dataclass
class RoBERTaConfig:
    """All configuration parameters for the RoBERTa training pipeline."""
    ...
    # ── Tokenization ─────────────────────────────────────────────────────────
    # PROFILING: 79% of texts exceed 256 tokens (mean=426, median=407).
    # Every batch will be near-fully dense at 256 tokens.
    # Dynamic padding gives <5% benefit here — keep static padding for simplicity.
    max_length: int = 512
    ...
    # ── Output directories ───────────────────────────────────────────────────
    output_dir: Path = PROJECT_ROOT / "outputs" / "roberta"
    checkpoint_dir: Path = PROJECT_ROOT / "outputs" / "roberta" / "checkpoints"
    tokenizer_dir: Path = PROJECT_ROOT / "outputs" / "roberta" / "tokenizer"
    predictions_dir: Path = PROJECT_ROOT / "outputs" / "roberta" / "predictions"
    reports_dir: Path = PROJECT_ROOT / "outputs" / "roberta" / "reports"
    figures_dir: Path = PROJECT_ROOT / "outputs" / "roberta" / "figures"
    logs_dir: Path = PROJECT_ROOT / "outputs" / "roberta" / "logs"
