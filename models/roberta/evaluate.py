...
evaluate.py
===========
Standalone evaluation script for a fine-tuned RoBERTa checkpoint.

Usage:
    python models/roberta/evaluate.py --checkpoint outputs/roberta/checkpoints/checkpoint-best

If no --checkpoint is given, loads the best model saved by train.py at
``outputs/roberta/tokenizer`` (the final save_pretrained location).
...
# Allow ``python models/roberta/evaluate.py`` to find sibling modules
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate a fine-tuned RoBERTa checkpoint on the test set."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default=str(CFG.tokenizer_dir),
        help="Path to model checkpoint directory (default: outputs/roberta/tokenizer).",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def main() -> None:
    ...
    # ── Build Trainer (no train/val datasets needed for eval) ─────────────
    trainer = build_trainer(
        model=model,
        tokenizer=tokenizer,
        train_dataset=tokenized["train"],   # required arg, not actually used
