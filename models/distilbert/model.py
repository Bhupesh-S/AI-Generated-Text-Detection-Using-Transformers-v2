...
model.py
========
DistilBERT model loading and configuration for binary text classification.
...
def load_model(
    ...
    """
    Load (or fine-tuned checkpoint of) ``distilbert-base-uncased`` for binary
    sequence classification.

    Args:
        model_name_or_path : Hugging Face model ID or local checkpoint path.
                             Defaults to ``CFG.model_name`` (``"distilbert-base-uncased"``).
        num_labels         : Number of output classes (default 2).

    Returns:
        :class:`transformers.DistilBertForSequenceClassification` on the best
        available device.
    """
