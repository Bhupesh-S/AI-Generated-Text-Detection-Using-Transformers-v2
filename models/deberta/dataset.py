...
dataset.py
==========
Dataset loading, splitting, and tokenization for the DeBERTa-v3 pipeline.
...
Responsibilities:
  ...
  - Batch tokenization via ``AutoTokenizer`` (SentencePiece compatible).
...
def tokenize_datasets(dataset_dict: DatasetDict, tokenizer: PreTrainedTokenizerBase) -> DatasetDict:
    """
    Tokenize all splits in the DatasetDict in place using the provided tokenizer.

    Optimizations:
      - Runs with batched mapping (batch_size=1000) for CPU utilization.
      - Uses fast Rust tokenizer implementation under the hood where applicable.
      - Removes raw text column to reduce RAM storage footprint.

    Args:
        dataset_dict : Loaded train/val/test splits.
        tokenizer    : Configured AutoTokenizer instance.
    ...
    logger.info("Tokenizing all splits with batch_size=1000...")
    tokenized = dataset_dict.map(
        ...
        desc="Tokenizing",
        remove_columns=[CFG.text_column],  # Free RAM of original text strings
    )
