...
model.py
========
DeBERTa-v3 model loading and configuration for binary text classification.

Responsibilities:
  - Load ``microsoft/deberta-v3-base`` via ``AutoModelForSequenceClassification``.
  - Attach label ↔ id mappings.
  - Provide a parameter-count summary.
  - Set ``use_safetensors=True`` to prevent CVE-2025-32434 torch.load block.
...
def load_model(
    ...
    """
    Load (or fine-tuned checkpoint of) DeBERTa-v3 for binary classification.

    Args:
        model_name_or_path : Hugging Face model ID or local checkpoint path.
                             Defaults to ``CFG.model_name`` (``"microsoft/deberta-v3-base"``).
    ...
    logger.info(f"Loading model: {model_name_or_path} (num_labels={num_labels}, use_safetensors={CFG.use_safetensors})")
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        use_safetensors=CFG.use_safetensors,   # Safeguard against torch.load block
