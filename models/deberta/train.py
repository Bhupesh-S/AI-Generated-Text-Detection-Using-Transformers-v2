...
train.py
========
Main entry point orchestrating the DeBERTa-v3 fine-tuning training pipeline.

Features:
  - Step-by-step pipeline orchestration.
  - Dry run option to verify configuration correctness.
  - Native Ampere BF16 execution to avoid DeBERTa-v3 gradient crashes.
  - Safetensors checkpoint loading to bypass pickle guards.
  - Performance comparisons with RoBERTa.

Run with:
    python models/deberta/train.py
    python models/deberta/train.py --dry-run  (verifies pipeline in 30 seconds)
...
# Set up logging to console and logs/train.log
CFG.create_dirs()
logger = get_logger(
    __name__,
...
# ---------------------------------------------------------------------------
# Training Orchestrator Pipeline
# ---------------------------------------------------------------------------

def run_pipeline(dry_run: bool = False) -> None:
    """Orchestrate the DeBERTa fine-tuning training and validation pipeline."""
    pipeline_start = time.time()

    if dry_run:
        print_section("RUNNING IN DRY-RUN MODE (MOCK RUN)")
        logger.info("Setting max_steps=5 to quickly verify pipeline execution correctness.")
    ...
    max_steps_override = 5 if dry_run else None
    ...
    logger.info("Model and tokenizer saved successfully via save_pretrained().")
    ...
    report_path = CFG.reports_dir / "DeBERTa_Report.md"
    generate_report(
    ...
    print_section("Step 11 - Triggering Cross-Model Comparison")
    try:
        generate_comparison()
    except Exception as comp_err:
        logger.error(f"Failed to generate cross-model comparisons: {comp_err}")
    ...
    logger.info("  python models/deberta/inference.py --interactive")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DeBERTa Fine-Tuning Pipeline Orchestrator")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Quick check with max_steps=5 to verify implementation details."
    )
